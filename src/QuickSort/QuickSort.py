...
def partition(array, low, high, cycle, steps):
    pivote = array[high]  # Elegir el último elemento como pivote
...
def quickSort(array, low, high, cycle = 0, steps = 0):
    ...
    if low < high:
        ...
        piv, cycle, steps = partition(array, low, high, cycle, steps)
        
        # Ordenar los elementos por separado antes y después de la partición
        array, cycle, steps = quickSort(array, low, piv - 1, cycle, steps)
        array, cycle, steps = quickSort(array, piv + 1, high, cycle, steps)
        steps += 3
